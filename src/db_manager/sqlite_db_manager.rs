...
use rusqlite::{self, params, Connection, OptionalExtension, Result};
use semver::Version;
...
impl DbManager for SqliteDbManager {
    ...
    async fn new(config: &DbConfig) -> Result<SqliteDbManager, Error> {
        let path = config.db_file_path.clone();
        tracing::info!("create and/or open database: {:?}", config.db_file_path);

        let pool = tokio::task::spawn_blocking(|| {
            r2d2::Pool::new(SqliteConnectionManager::file(path)).map_err(Error::Pool)
        })
        .map_err(Error::Join)
        .await??;
...
impl SqliteDbManager {
    ...
    fn get_users(&self) -> Result<Vec<User>, Error> {
        let conn = self.pool.clone().get().map_err(Error::Pool)?;
        let mut stmt = conn
            .prepare("SELECT id, login, name FROM users")
            .map_err(Error::Db)?;
        let ret = Ok(stmt
            .query_map([], |row| {
                Ok(User {
    ...
    #[tracing::instrument(skip(self, name))]
    fn get_entry(&self, name: &str) -> Result<Option<Entry>, Error> {
        let conn = self.pool.clone().get().map_err(Error::Pool)?;
        let mut stmt = conn
            .prepare("SELECT versions, owner_ids FROM entries WHERE name = ?1")
            .map_err(Error::Db)?;
        stmt.query_row(params![name], |row| {
            Ok(Entry {
                versions: serde_json::from_str(&row.get::<_, String>(0)?)
                    .expect("versions is stored as a serialized JSON object."),
                owner_ids: serde_json::from_str(&row.get::<_, String>(1)?)
                    .expect("owner_ids is stored as a serialized JSON object."),
            })
        })
        .optional()
        .map_err(Error::Db)
    ...
    {
        let mut users: Vec<User> = self.get_users()?;
        users.sort_by_key(|u| u.login.clone());
    ...
    async fn entry(&self, name: &str) -> Result<Entry, Error> {
        ...
        self.get_entry(&name).map(Option::unwrap_or_default)
    }
    ...
    fn insert_entry<'a>(&self, name: &str, entry: Entry) -> Result<(), Error> {
        let norm_name = normalized_crate_name(name);

        let conn = self.pool.clone().get().map_err(Error::Pool)?;
        conn.execute(
            "INSERT INTO entries (name, versions, owner_ids) VALUES (?1, ?2, ?3)",
            params![
                norm_name,
                serde_json::to_string(&entry.versions).map_err(Error::Serialization)?,
                serde_json::to_string(&entry.owner_ids).map_err(Error::Serialization)?
            ],
        )
        .map(|_| {})
        .map_err(Error::Db)
    ...
    fn insert(&self, key: impl AsRef<[u8]>, value: impl Serialize) -> Result<(), Error> {
